...
void main() {
    ...
    vDisplacement = cnoise(position + vec3(0.5 * u_time));
  
    // Define the rotation axis (for example, rotating around the Y-axis)
    ...
    vec3 newPosition = (rotationMatrix * vec4(position, 1.0)).xyz + normal * (u_intensity * vDisplacement);
